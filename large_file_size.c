...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout, GREEN"[+]"RESET" Verifying command line arguments \n");

	if (argc < 2)
	{
		fprintf(stderr, RED"[-]"RESET" Usage: %s <filename> <expand_size> (in MB) \n", argv[0]);
		exit(0x10);
	}
	...
	if (argc != 3 )
	{
		...
		fprintf(stdout, GREEN"[+]"RESET" File will be expanded by default size : %ld Bytes\n", file_size);
	}
	...
	else
	{
		...
		fprintf(stdout, GREEN"[+]"RESET" File will be expanded by %ld Bytes \n", file_size);
	}


	// open the file
	fprintf(stdout, GREEN"[+]"RESET" Opening file : %s \n", argv[1]);
	int fd = open(argv[1],  O_WRONLY | O_CREAT | O_EXCL, 0777);
	if (fd == -1)
	{
		fprintf(stderr, RED"[-]"RESET" Error: while opening '%s' \n", argv[1]);
		exit(0x11);
	}
	...
	if (old_position == -1)
	{
		fprintf(stderr, RED"[-]"RESET" Error: while getting current position \n");
		exit(0x12);
	}	
	
	fprintf(stdout, GREEN"[+]"RESET" Current position : 0x%08lx \n", old_position);
	...
	if (new_position == -1)
	{
		fprintf(stderr, RED"[-]"RESET" Error: lseeking to %ld bytes from the current position", file_size - 1);
		exit(0x13);
	}
	
	fprintf(stdout, GREEN"[+]"RESET" Setting current position to 0x%08lx \n", new_position);

	// Write a \x00 byte on that location 
	fprintf(stdout, GREEN"[+]"RESET" Writing 0x%02x at offset 0x%08lx from starting of file \n", 0x00, new_position);
	write(fd, "\x00" , 1);
